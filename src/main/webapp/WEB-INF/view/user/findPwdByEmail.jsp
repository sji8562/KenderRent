					...
					<div class="row">
						...
						<div class="col-lg-4 get_in_touch_col">
							<div class="get_in_touch_contents">
								<h1>비밀번호 찾기</h1>
								<br> <br>
								<form action="/user/find-pwd-by-email" method="post" onsubmit="return validateForm()">
									<label for="userName" class="label_txt">이름</label> <input type="text" id="username"
										name="username" value="${user.userName}" class="input_txt" style="width: 217px"
										required> <br>
									<!-- <span>이메일 </span> -->
									<div class="form-group last mb-4 email_input">
										<label for="memail" id="mailTxt">이메일을 입력해주세요</label> <input type="text"
									...
									<div class="form-group last mb-4 check_input">
										<label for="memailconfirm" id="memailconfirmTxt">인증번호를
											입력해주세요</label> <input type="text" class="form-control" id="memailconfirm">
									</div>
								</form>
									...
									$checkEmail
										.click(function () {
											$
												.ajax({
													type: "POST",
													url: "find-pwd-by-email/mailAuthorize",
													data: {
														"email": $memail.val()
													},
													success: function (data) {
														alert("해당 주소로 인증번호 발송이 완료되었습니다. \n 확인부탁드립니다.")
														console.log("data : "
															+ data);
